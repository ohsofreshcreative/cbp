...
Sam nawias **ustala skąd treść**: podstrona `(CPT)` = wpis CPT, nie Page; warstwa `(CPT)` = listing z CPT; warstwa `(Options)` = treść z istniejącej Options (gdy taka strona jest w motywie). Nie zgaduj Page, gdy Figma mówi CPT.
