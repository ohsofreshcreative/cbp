...
use App\Support\OsfImportConfig;
...
$example = dirname(__DIR__) . '/examples/page.json';
...
expect_true(is_string($payload->sourceDir) && str_ends_with($payload->sourceDir, 'examples'), 'sourceDir z pliku JSON');
...
expect_true(is_readable(PageImportPayload::blockClassPath('hero')), 'Hero.php jest w app/Blocks motywu');
...
expect_true(str_ends_with(str_replace('\\', '/', OsfImportConfig::blocksPath()), '/app/Blocks'), 'kit szuka bloków w app/Blocks, nie w osf-import');
