...
4. Pomiń chrome: `menu`, `header`, `footer`. Opakowania **nie są** nazwami bloków — każdy `Frame` + numer (`Frame 211`, `Frame 476`, `Frame 4xx`, …), `Group`, `Content`, `Banner & Problem`, `__wrapper`. Wejdź do środka i bierz **nazwane dzieci-sekcje**. Przykład: `Frame 211` z `About` i `Numbers` = bloki `about` + `numbers`, nie `frame211`. Jedno nazwane dziecko w opakowaniu = ta sekcja (`Content` → `Problem` = `problem`).
5. `get_design_context` na **każdej** nazwanej sekcji, którą składasz — nie na opakowaniu `Frame NNN`. Screenshot całej strony nie zastępuje kontekstu sekcji.
